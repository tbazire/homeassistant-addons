...
## [0.10.0-dev] - 2026-09-24

Adds the nine EEBUS HVAC use cases (domestic hot water + room heating), read
and write, grafted from the [volschin/eebus-go](https://github.com/volschin/eebus-go)
fork (v0.7.1-0.20260731151816-4b2730e630aa — same spine-go/ship-go pins as our
vendored tree, so the graft stays confined to `eebus-go`).
...
- 🧪 **Field validation wanted before the production migration**: run against
  a real HVAC device with `log_level: debug` and report the measurement
  description inventory (`desc id=… type=… scope=…` — flow/return temperature
  scopes vary per vendor) and the emitted `uc_signal` lines.

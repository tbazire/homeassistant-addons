...
- **DHW circuit → `water_heater`**: the bridge composes the CDT number
  ...
  sensor via MDSF); the start/stop controls may follow after field feedback.
...
> 🧪 **Field validation wanted (0.10.0-dev):** the exact description inventory
> (flow temperature / return temperature scopes, overrun status values) varies
> per vendor. Run with `log_level: debug` on a real device and report the
> `desc id=… type=… scope=…` lines and the emitted `uc_signal` lines so the
> mapping can be confirmed before the production migration.
